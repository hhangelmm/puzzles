<?php
$num_arr = array(array(08, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 08),
...
$num1=0;
foreach($num_arr as $row=>$arr){
	foreach($arr as $low=>$v){
		if(isset($num_arr[$row][$low])&&isset($num_arr[$row][$low+1])&&isset($num_arr[$row][$low+2])&&isset($num_arr[$row][$low+3])){
			$num = $num_arr[$row][$low]*$num_arr[$row][$low+1]*$num_arr[$row][$low+2]*$num_arr[$row][$low+3];
			if($num1<$num)
				$num1 = $num;		
		}
	}
...
for($i=0;$i<20;$i++){
	for($j=0;$j<20;$j++){
		if(isset($num_arr[$j][$i])&&isset($num_arr[$j+1][$i])&&isset($num_arr[$j+2][$i])&&isset($num_arr[$j+3][$i])){
			$num = $num_arr[$j][$i]*$num_arr[$j+1][$i]*$num_arr[$j+2][$i]*$num_arr[$j+3][$i];
			if($num2<$num)
                                $num2 = $num;
...
for($i=0;$i<20;$i++){
        for($j=0;$j<20;$j++){
                if(isset($num_arr[$j][$i])&&isset($num_arr[$j+1][$i+1])&&isset($num_arr[$j+2][$i+2])&&isset($num_arr[$j+3][$i+3])){
                        $num = $num_arr[$j][$i]*$num_arr[$j+1][$i+1]*$num_arr[$j+2][$i+2]*$num_arr[$j+3][$i+3];
                        if($num3<$num)
                                $num3 = $num;
